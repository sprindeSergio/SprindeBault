...
def insert_hardware(db, id, codeName, IP, nombre, password, hardware_type_id, localizacion):
    query = """
    INSERT INTO hardware (id, CodeName, IP, nombre, password_hash, Hardware_type_Id, Localizacion) 
    VALUES (%s, %s, %s, %s, %s, %s, %s)
    """
    with db.cursor() as cursor:
        cursor.execute(query, (id, codeName, IP, nombre, password, hardware_type_id, localizacion))
    db.commit()